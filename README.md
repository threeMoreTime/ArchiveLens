...
| HTML 离线审阅报告 | ✅ | 永久序号、出处→文字→原图、全量导航、筛选分页、单命中标注与 A4 打印离线回归 |
...
- 数据库 schema 升级前自动建立通过完整性与 SHA-256 校验的本地明文备份，默认保留最近
  3 份；降级必须恢复匹配旧版 schema 的升级前备份，旧程序不得直接打开新库；
...
设置页底部的版本号是隐藏开发者入口：3 秒内连续点击或键盘激活 7 次即可开启开发者模式，第 5、6 次提示剩余次数，第 7 次开启并在设置侧栏显示“开发者”入口。开发者页面提供“退出开发者模式”，退出后入口从 DOM 移除，相关受限 IPC 立即恢复门禁。

- 连续点击版本号只是隐藏入口，不是账户或权限系统；应用假定由当前 Windows 账户所有者本地使用。
- 生产环境仍拦截 F12 与 Ctrl+Shift+I；只有开发者页面的按钮才能打开当前窗口的 DevTools。
- “复制 AI 错误调试信息”包含 OCR 正文、用户名、完整路径、文件名、原始错误、调用栈和最近 300 行日志；所有诊断信息只写入本机剪贴板，绝不自动上传。
- 开发者模式只新增 Electron 本地 IPC，不改变 Python Engine 协议；设置文件版本保持 4，旧版本会忽略新增的 `developer` 字段。

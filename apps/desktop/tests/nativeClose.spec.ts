...
function createHarness(options: { approvedQuit?: boolean; shutdownFlowRunning?: boolean } = {}) {
  ...
  const lifecycle = {
    getState: vi.fn(() => ({ approvedQuit: options.approvedQuit ?? false, shutdownFlowRunning: options.shutdownFlowRunning ?? false })),
    requestClose: vi.fn(async () => ({ requiresAction: true, activeTask: { task_id: "task-1", status: "running" } })),
  ...
  installNativeCloseHandler({
    win: { on: (_event, next) => { listener = next; } },
    lifecycle,
    prompt,
    logger,
  });
  return { lifecycle, prompt, logger, close: async () => {
    const preventDefault = vi.fn();
    listener?.({ preventDefault });
    await vi.waitFor(() => expect(lifecycle.requestClose).toHaveBeenCalledTimes(options.approvedQuit || options.shutdownFlowRunning ? 0 : 1));
...
describe("native window close lifecycle", () => {
  ...
  it("does not open a duplicate native decision while a flow is active", async () => {

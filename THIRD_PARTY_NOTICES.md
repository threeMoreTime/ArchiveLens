...
| 组件 | 许可证 | 说明 |
| --- | --- | --- |
| [PyMuPDF / fitz](https://pymupdf.readthedocs.io/) | **AGPL-3.0**（或商业授权） | ⚠️ AGPL 具传染性；正式发布前必须确认采用 AGPL 合规路径或购买商业授权。**当前未随二进制分发 PyMuPDF 的源码义务履行方案，发布前必须解决。** |
...
> ⚠️ **本轮尚未把以上模型/语言包/Tesseract/DjVuLibre 二进制纳入安装包**。
> 当前的 packaged Engine 在 `diagnostics.run` 中对语言包检测为 FAIL/WARN（依赖宿主已安装）。
> 合法随包分发前必须：(1) 完成许可证核实；(2) PyMuPDF 的 AGPL 决策；(3) DjVuLibre 的 GPL 评估或动态链接方案。
...
1. **PyMuPDF AGPL**：必须确认合规路径（AGPL 对应源码披露 / 商业授权 / 替换为许可更宽松的 PDF 库）。
2. **DjVuLibre GPL-2.0**：评估随包分发的传染性影响，或维持「不随包、应用内安装」。
3. **模型/语言包**：逐文件核实来源与许可证后再分发。

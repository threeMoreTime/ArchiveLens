...
项目：ArchiveLens
可信基线：955646a67bebdebeeff7ed46ef2bdcea75d839e8
组合代码验证候选：e878ccca7ee394006ef28798497433103630ebcc
集成与报告分支：codex/integration/stable-candidate-20260717
...
所有“已修复”均已进入本地组合候选并完成组合验证；由于未合并到 main，不能视为当前 main 已具备该修复。

### 2026-07-17 组合执行补录

| 项目 | 结果 |
| --- | --- |
| 隔离 worktree | `F:\.zcf\OCR\stable-candidate-20260717` |
| 集成分支 | `codex/integration/stable-candidate-20260717` |
| 集成起点 | `9fcc6ce3ccd74b3beea8fc6e73f7c8a76fe95482`，已继承 P0、Electron 43.1.1、许可证门禁、零成本发布门禁、HR-01、统一 OCR 模型、持久化 OCR 索引和简繁检索 UI |
| 新纳入修复 | 资源协议联接越界、Sidecar 启动/导出时限、校对 E2E 长路径、校对输入快捷键、渲染状态一致性、日志轮转，共 6 个提交 |
| 组合代码 SHA | `e878ccca7ee394006ef28798497433103630ebcc` |
| 零成本门禁 | 29/29 步通过；证据目录 `.tmp/release-gate/e878ccca7ee394006ef28798497433103630ebcc/20260717T045107Z` |
| 测试规模 | 桌面 Vitest 24 文件、138 项通过；Python 278 项通过、1 项因无 `pwsh` 跳过；Playwright 25/25 通过 |
| 制品验证 | Setup、Portable、`win-unpacked`、打包引擎、manifest、SHA256SUMS、安装/启动/卸载、Portable 清理和同 SHA 链全部通过 |
| 费用与外部动作 | 金额 0；未推送、未创建 PR、未合并 main、未签名购买、未部署、未正式发布 |
| 数据安全边界 | 测试使用隔离目录，未读写真实用户数据库；候选 schema 为 v7、main 为 v6，源码回退到 main 不能作为已验证的数据回滚方案 |

组合验证期间记录到三项可复现的前置条件问题：Electron 首次并发加载造成一次本地二进制安装目录竞争；新 worktree 未准备锁定 OCR 模型；未打包时 `vertical.spec.ts` 找不到 `win-unpacked/ArchiveLens.exe`。分别通过串行预热、锁文件模型准备和完整本地门禁建立打包候选后原命令复跑，最终组合门禁全部通过，未通过修改断言、跳过测试或降低标准规避失败。
...
- 核心业务闭环：导入或扫描 → 配置检索 → OCR 执行 → 查看进度与恢复 → 校对 → JSON/HTML 导出 → 历史管理，主链路已形成；HR-01 已把继续操作收紧到可信恢复状态，历史候选 32f0065 还完成了 993 页真实 PDF 全量 OCR。
- 真实用户使用条件：组合候选适合开发者或受控测试者在已知限制下使用；当前 main 尚未包含本轮 P0/P1、HR 与统一简繁检索成果，不适合直接声明面向普通用户稳定可用。
- 稳定发布条件：本地零成本候选门禁已通过，但稳定公开发布仍不具备。正式发布未获授权，跨版本升级/回滚未用上一可信安装包验证，公开分发许可证批准仍为显式阻塞。
- 问题总数：26。
- 优先级：P0 1；P1 8；P2 13；P3 4。
- 当前执行分类：可自动实施 12；需要人工审核 13；被其他任务阻塞 0；状态无法确认 1。HR-02 和 HR-04 的技术实施已完成，但正式发布授权和公开许可证批准仍属于人工审核。
- 已创建本地分支：13 个，其中 11 个原实现分支、1 个原报告分支、1 个组合集成分支。
- 已完成开发批次：12 个；B-01 至 B-11 的有效成果已在 B-12 组合候选中汇合并完成同 SHA 门禁，尚未合并到 main。
- 最严重三项：
  1. 当前 main 的生产导航判断在空开发服务器 URL 下可接受任意 URL，同时生产 CSP 仍允许开发期能力；已在组合候选修复并验证，但尚未合并。
  2. 稳定公开发布仍被正式发布授权、真实跨版本升级/回滚和公开许可证批准三项门禁阻塞，不能把本地门禁通过描述为已可发布。
  3. 候选数据库 schema 已升至 v7，而 main 仍为 v6；升级测试通过，但从候选源码回退到 main 的数据兼容性没有验证，不能把 Git 回退等同于数据回滚。
- 推荐下一步：人工审阅单一组合分支 `codex/integration/stable-candidate-20260717`；审核通过后再单独授权 PR 或合并。公开发布前仍须完成跨版本升级/回滚、冻结候选许可证批准和正式发布授权。
...
| 最终可信基线 | main = origin/main = 955646a67bebdebeeff7ed46ef2bdcea75d839e8，ahead/behind 0/0，工作区干净 |
| 多 worktree | 已确认存在；审阅与集成阶段共创建 13 个 worktree，其中新增组合 worktree 位于 F:\.zcf\OCR\stable-candidate-20260717。另有多个历史 prunable 记录，未清理 |
...
| 当前已知阻塞 | 组合候选尚待人工代码审核且未合并；稳定公开发布缺正式授权、跨版本升级/回滚证据和公开许可证批准 |
...
| 历史候选真实文档检索 | 32f0065 上完成 993/993 页、失败 0；“亏空”精确命中 0；该大文档任务未在 e878ccca 上重复执行 | VERIFIED（32f0065）/ NOT VERIFIED（e878ccca 重跑） |
...
| UI 与交互 | 已完成但未充分验证 | 视觉体系一致、主操作清楚；组合候选已修复校对输入快捷键误触和状态文案缺口，最小窗口下校对密度仍高 |
| 工程质量 | 部分完成 | 进程边界和 IPC 契约较清晰，但存在超大模块、无真实 lint、单 bundle 偏大 |
| 数据与安全 | 部分完成 | SQLite 迁移、事务、WAL、路径校验基础较好；组合候选已修复导航/CSP 与资源联接越界问题；静态数据未加密，v7 → v6 数据回退未验证 |
...
| OCR 执行与生命周期 | 已完成并验证 | 启动、暂停、继续、取消、可信恢复状态、进度；993 页真实 PDF 完成 | 组合候选尚未合并；磁盘满和多小时中断恢复未验证 | lifecycle E2E、HR-01 测试、真实文档任务 |
...
| 执行并管理长任务 | 创建 → 运行 → 查看进度 → 暂停/继续/取消 → 异常恢复 | 已完成并验证 | 中 | HR-01 已收紧 failed/stale 合同并进入组合候选；尚未合并，磁盘满/强制崩溃后的 993 页恢复未验证 |
| 搜索并校对结果 | 任务 → 校对 → 检索/筛选 → 查看页证据 → 决策/备注/旋转 | 已完成并验证 | 高 | 原 main 输入时会触发全局快捷键；最小窗口信息密集 |
...
原 B-01 至 B-08 均可从 955646a6 独立审阅；B-09 基于 B-01，B-10 基于 B-09，B-11 基于 B-10。B-12 以包含发布链、HR-01 和统一简繁 OCR/索引/搜索的 `9fcc6ce3` 为起点，将其余六个独立修复按安全与依赖顺序纳入，形成单一组合候选。所有分支仍未合并到 main。
...
| B-12 | codex/integration/stable-candidate-20260717 | P0/P1/P2 | B-01 至 B-11、统一简繁 OCR/索引/搜索及六个独立修复的组合 | 已完成，待人工审核 | 9fcc6ce3、1a3ac92c、25743e2c、6e73b92c、244c7b6f、0fb8f848、e878ccca | 39 项针对性测试、138 项桌面单测、278 项 Python、25 项 E2E、构建、打包、安装/卸载和 29 步同 SHA 零成本门禁通过 |

### 分支依赖和组合注意

- 功能依赖：B-12 已继承 B-01、B-09、B-10、B-11、HR-01 及统一简繁 OCR 分支链；不得再重复合入这些祖先分支。
...
- 人工审核入口：只审核 B-12 即可查看最终组合差异；原分支保留为证据和提交来源，不应分别重复合并。
...
- 风险：重叠文件无文本冲突不代表语义无冲突；候选 schema v7 与 main v6 的数据回退不兼容；构建/安装会产生大型忽略产物和隔离进程。
...
- 遗留问题：人工代码审核、跨版本升级/回滚、冻结候选公开许可证批准、正式发布授权、真实 lint、e878ccca 上的 993 页大文档重跑及其他人工审核项。
- 是否可进入人工代码审核：是；应以 B-12 为唯一组合审核入口。

### 真实文档测试：T-01

- 测试目标：使用最终候选打包引擎，在工作区文件 `F:\OCR\乾隆朝上谕档 第1册(乾隆01年至09年).pdf` 中精确检索简体关键词“亏空”。
...
- 性能：结果分页与任务进度已实现；993 页 PDF 在 1,854.406 秒内完成、失败 0，提供了真实长文档吞吐证据。单 bundle 约 1.18 MB、超大文件夹预检和多任务并发预算仍缺失。
- 稳定性：生命周期 E2E 较强；B-03 修复启动超时清理，B-05 限制日志；历史候选 32f0065 连续处理 993 页并正常 shutdown，组合候选 e878ccca 完成 29 步门禁。磁盘满、强制崩溃后大任务恢复和多小时压力仍未验证。
...
- 构建与发布：最终候选 32f0065 的引擎、Setup、Portable、manifest 和哈希同源通过，签名状态为 NotSigned；不能据此宣布稳定 release，跨版本升级/回滚、许可证批准和正式发布授权仍缺失。
...
- 被依赖阻塞：无。原 B-QA-01 已在 B-11 重写并通过；剩余工作属于未集成、环境证据不足或人工门禁。
- 环境无法验证：上一可信安装版本到 e878ccca 的真实升级/回滚；pwsh 专用 Python 测试在本机跳过；未进行屏幕阅读器、磁盘满、受限 Windows 用户和多小时/多任务压力测试。
...
- 跨分支集成：B-12 已完成全部有效成果的本地组合与验证；剩余动作是人工代码审核以及另行授权的 PR/合并，不再被技术分支依赖阻塞。
- 真实文档结果边界：“亏空”精确命中 0，不自动扩展为繁体“虧空”、模糊匹配或人工校勘结论。

## 13. 分阶段路线图

| 阶段 | 核心目标 | 主要任务 | 可自动实施 | 需人工审核 | 完成标志 |
| --- | --- | --- | --- | --- | --- |
| 1. 可信基线和 P0 | 消除立即安全风险 | B-01 已进入 B-12 并验证 | 已完成 | 安全代码审核 | B-12 安全 E2E、全量测试和 build 已通过 |
| 2. 核心业务闭环 | 稳定校对、sidecar、导出与恢复 | B-02、B-03、B-04、B-08 已进入 B-12 | 已完成 | 组合代码审核 | 正常/失败/中断/可信恢复合同和 E2E 已通过 |
| 3. 用户体验与效率 | 完整状态、可靠资源和可理解反馈 | B-06、B-07 已进入 B-12；评估小窗和大任务 | 已完成明确修复 | HR-05、HR-07、HR-11 | 目标视口与大任务验收标准明确 |
| 4. 工程质量与稳定性 | 控制磁盘增长并补门禁 | B-05 已进入 B-12；后续建立真实 lint/覆盖率预算 | 日志轮转已完成 | HR-06、HR-08、HR-10 | 门禁无降级，长期/删除异常可恢复 |
| 5. 构建发布与稳定版 | 建立同 SHA 可复验制品链 | B-09 至 B-12 已完成；补跨版本升级/回滚 | 零成本门禁已完成 | HR-02 正式发布、HR-04 公开批准、HR-09 | 集成候选同 SHA 制品、安装已通过；升级、回滚和授权仍待完成 |
...
- H-REC-01 和 H-DEP-01 已有批准决策并进入组合候选。
...
- 全部 B-01 至 B-11 组合分支的完整零成本门禁；干净同 SHA win-unpacked、Setup、Portable smoke。B-12 已满足，合并后候选仍需按新 SHA 重建验证。
- HR-01 合同对应的成功、校验失败、中断、重复调用和恢复回归。
- 至少保留一项真实大文档检索回归；本轮 993 页“亏空”精确检索可作为性能与稳定性基线。

### 必须具备的数据安全和恢复能力

- 任务数据库迁移/回滚、future schema 拒绝、WAL 恢复保持通过。
- 候选 schema v7 到 main schema v6 的源码降级不是已验证回滚；需要上一可信安装包和备份副本完成真实跨版本升级/回滚演练。
...
- Setup 和 Portable 在隔离 Windows 环境启动并执行关键 smoke；B-12 组合候选已通过，合并后的新 SHA 仍需重跑。
...
- 日志轮转、状态一致性和资源协议 containment 虽为 P2，但收益高、风险低，已进入 B-12；是否合入由人工代码审核决定。

## 15. Git 操作记录

| 操作 | 结果 |
| --- | --- |
| 创建本地实现分支 | 原 11 个实现分支，加 1 个 `codex/integration/stable-candidate-20260717` 组合分支 |
| 创建本地报告分支 | 原 `codex/docs/comprehensive-audit-20260716` 保留；报告内容以未提交方式取入 B-12 后更新并形成单一 docs 提交 |
| 创建本地提交 | B-12 相对 main 含 18 个代码/测试/build 提交；本报告形成后再增加 1 个 docs 提交 |
| 基线 main 变更 | 本轮未在 main 写入；外部并发流程将用户原改动提交并推进至 955646a6 |
| 暂存用户原有改动 | 未执行 |
| 推送远程 | 未执行 |
| 创建 Pull Request | 未执行 |
| 合并默认分支 | 未执行 |
...
1. 当前项目真实状态：ArchiveLens 是具有真实 OCR、持久化、任务生命周期、校对和导出能力的 Alpha 产品，不是 Mock；历史候选成功处理 993 页真实 PDF，B-12 又完成全部分支组合和 29 步门禁。当前 main 仍未包含本轮成果，不能把候选状态当成默认分支状态。
2. 是否适合继续增加新功能：不建议立即扩张核心功能。应先人工审阅 B-12 并处理合并授权；P3 新功能可后置。
3. 已自动修复：原 9 项明确缺陷，以及 HR-01 恢复合同、HR-03 Electron 第一阶段、统一简繁 OCR/索引/搜索和旧 vertical E2E 可复现问题均已进入组合候选；HR-02/HR-04 的批准技术范围也已实现，但其人工发布边界仍保留。
4. 已建立分支：11 个原实现分支、1 个原报告分支和 1 个 B-12 组合分支；最终人工审核应以 B-12 为唯一合并候选，不重复合入祖先分支。
5. 已完成可审核批次：B-01 至 B-12；组合代码 SHA `e878ccca` 的单元、引擎、E2E、打包、安装和发布链均已实际验证。
6. 仍需人工决定：HR-02 的正式发布授权、HR-04 的冻结候选许可证批准，以及 HR-05 至 HR-11。HR-01 和 HR-03 当前阶段已决定并执行。
7. 当前最优先人工审核：审阅 B-12 的最终组合差异，重点关注恢复合同、schema v7、统一简繁索引、资源路径安全和发布门禁；稳定版前再完成跨版本升级/回滚、HR-04 许可证批准和 HR-02 正式发布授权。
8. 距离稳定版缺少：人工代码审核与合并决策、跨版本升级/回滚、冻结候选许可证批准、正式发布授权、HR-08 删除一致性和 HR-09 隐私边界确认；不再缺全部分支组合回归。
9. 是否推送或创建 PR：否；也未合并、发布、签名或部署。
10. 未提交或未验证内容：本报告将在 B-12 形成单一 docs 提交；忽略的模型、构建、安装和门禁证据不属于 Git 改动。跨版本升级/回滚、e878ccca 上的 993 页真实文档重跑、屏幕阅读器、磁盘满和真实用户验收未验证。

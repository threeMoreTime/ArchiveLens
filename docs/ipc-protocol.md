...
- `protocol_version` 当前为 `4`，TS（`packages/ipc-schema/src/index.ts`，Zod）与 Python（`archivelens_engine/protocol.py`）**必须一致**；v4 增加可清空的校对决定与事务化批量校对接口，不能与 v3 引擎混用；
- 不兼容版本必须显式失败（`PROTOCOL_MISMATCH`），不得静默继续。

## 请求

```json
{ "protocol_version": 4, "request_id": "<uuid>", "method": "app.info", "params": {} }
```

## 成功响应

```json
{ "protocol_version": 4, "request_id": "<uuid>", "ok": true, "result": {} }
```

## 错误响应

```json
{ "protocol_version": 4, "request_id": "<uuid>", "ok": false,
...
```json
{ "protocol_version": 4, "event": "task.progress", "task_id": "<uuid>",
...
```json
{ "protocol_version": 4, "event": "engine.ready", "task_id": null,
  "payload": { "engine_version": "0.1.0", "protocol_version": 4 } }
```

Main 只有在外层和 payload 的 `protocol_version` 都严格为整数 `4`，且
...
- `review.layoutContext` 按 `task_id + occurrence_id` 读取或重建当前命中的结构化版面上下文。
- `review.previewLayoutContext` 接受 `layout_mode` 和可选归一化版块矩形，只预览，不写入。
- `review.updateLayoutOverride` 将版面模式和可选矩形保存到当前来源页，或用 `clear=true` 清除该页修正；它不会修改 OCR 原文、永久序号、人工结论或备注。
- `review.rebuildLayoutContexts` 分批补齐旧任务上下文，可用 `priority_occurrence_id` 让当前命中及同页内容优先。

结构化上下文固定包含横排或竖排方向、置信度、目标 OCR 行、命中字符区间、最多三个空间邻项、源坐标与归一化坐标、候选版块及纯文本索引。竖排按右邻列、命中列、左邻列组织，横排按上一行、命中行、下一行组织，并隔离不同空间版块。为避免异常 OCR 页面造成无界计算与 IPC 载荷，分块只使用距命中最近的 256 行，候选版块最多返回 64 个且始终保留命中版块。自动识别置信不足时返回 `status="uncertain"` 且只含目标项。`results.query`、`search.hits` 和 HTML 导出复用同一结构；`results.query.layout_rebuild` 报告旧任务后台补齐进度。

### `review.updateDecision` / `review.updateDecisions`

单条和批量接口都接受 `confirmed`、`needs_review`、`rejected` 或 `null`。`null` 表示清除人工决定并恢复为待处理，不删除既有备注。单条响应返回最终决定和更新时间；批量响应额外返回服务端写入前的 `previous_decision`，供客户端按真实持久化状态建立撤销历史。

批量接口一次接受 1 至 10000 条变更，同一请求中 `occurrence_id` 必须唯一：

```json
{
  "task_id": "task_...",
  "operation_id": "3f74f65c-5e66-4d6e-96f8-44bb3fded74b",
  "changes": [
    { "occurrence_id": "occ_1", "decision": "confirmed" },
    { "occurrence_id": "occ_2", "decision": null }
  ]
}
```

Engine 在写入前验证任务状态、全部 occurrence 归属和参数，再用单个 SQLite 事务提交。任一 occurrence 不存在或任一变更无效时，整批失败且不写入部分结果。`operation_id` 是客户端为一次逻辑操作生成的 UUID；Engine 会在同一事务中保存请求摘要与原始响应，相同 UUID 与相同变更的重试返回原响应，不会再次改写决定；同一 UUID 携带不同变更会被拒绝。每个任务保留最近 10000 次逻辑操作的幂等响应，避免本地数据库随长期校对无界增长。成功响应回显 `operation_id`，并逐条返回 `occurrence_id`、最终 `decision` 与 `previous_decision`，顺序和请求一致。该合同保证批量确认、批量排除以及会话级撤销、重做不会出现半成功状态；桌面端单次选择同样限制为 10000 条。
...
- `results.query` 返回的每条 `OccurrenceItem` 包含任务内永久 `global_sequence`；默认按该序号升序。序号自 schema v12 起由正整数、任务内唯一和不可变约束保护，筛选、翻页、校对更新、扫描追加与 JSON/HTML 导出均复用同一值。
...
- 任务中心批量暂停、取消和删除是 Renderer 编排：最多 20 条，逐项以 `tasks.get` 重新核验后调用既有单任务方法。协议不新增批量写接口，任一失败不会改变其余调用的执行合同。

### 本地显示设置

- `settings.get` 与 `settings.update` 是 Renderer 到 Electron Main 的本地 IPC，不经过 Python Sidecar。
- 设置文件当前版本为 4；读取 v1/v2 时补入默认的 `search_script_scope="both"`，读取 v1 至 v3 的旧阅读方向与字符半径设置时迁移为 `layout_mode="auto"`，不会改写 OCR 数据。
...
- 扫描上下文偏好使用 `layout_mode = auto | vertical | horizontal`；旧 `context_direction` 与 `context_radius` 只保留在数据库中用于降级恢复，不再作为当前 Renderer 设置合同。
- `settings.update` 支持修改 `global` 全局默认，或按 `task_id` 保存/清除 `task` 覆盖；读取时返回全局值、任务覆盖和最终生效值。
- `settings.update` 还支持 `{ scope: "global", search_script_scope }`，取值为 `simplified | traditional | both`，默认 `both`；它决定新检索会话和新建扫描任务的默认范围。`tasks.create` 会把该值作为 `search_script_scope` 固化到任务，历史任务迁移时默认为 `both`。
...
开发者模式相关方法只用于 Electron Main↔Preload↔Renderer 的本地 IPC，**不经过 Python JSONL 协议**：

- `settings.getDeveloperMode` / `settings.setDeveloperMode`：读取与持久化 `developer.enabled`；
- `app.getVersion` / `app.getDeveloperSnapshot` / `app.reportRendererError`；
- `app.copyDiagnosticSummary`（`mode: "redacted" | "full"`）/ `app.copyAiDebugInfo`；
- `app.openRendererDevTools`、`app.openLogDirectory`（均受开发者模式门禁保护）。

这些方法不加入 `MethodNameSchema`，不影响 `PROTOCOL_VERSION`（仍为 4），设置文件版本也保持 4。相关 schema（如 `DeveloperSnapshotSchema`、`ClipboardCopyResultSchema`、`RendererErrorReportSchema`）定义在 `packages/ipc-schema`，仅供本地边界使用。门禁错误码 `DEVELOPER_MODE_REQUIRED` 与上报超限错误码 `DIAGNOSTIC_PAYLOAD_TOO_LARGE` 属于本地边界，不进入 Python `ErrorCode` 闭合枚举。

...
| 统一文字识别模型 | Python Engine 内的 `archivelens_models/` | PP-OCRv6 small，文件大小与 SHA-256 锁定 |
| DjVuLibre | `resources/native/djvulibre/` | 3.5.29，GPL-2.0-only |
| PDF / 图片与字形关系 | Python Engine 内 | PDFium、RapidOCR、ONNX Runtime、Pillow、OpenCC 1.2.0 |
...
`scripts/native-dependencies.lock.json` 固定所有下载地址和 SHA-256，包括唯一的
PP-OCRv6 small 文字识别模型。执行：
...
开发或 CI 只需要准备统一 OCR 模型时可执行：

```powershell
powershell -ExecutionPolicy Bypass -File scripts/prepare-native-runtime.ps1 -OcrOnly
```

OpenCC 作为锁定的 Python Engine 运行依赖进入 PyInstaller，用于本机字形族判断和
后续简体、标准繁体、台湾、香港索引；它不联网，也不接收用户文档。
候选模型、性能与内存门禁见 [`ocr-model-selection.md`](ocr-model-selection.md)。

...
- **共用前置（实际）**：B1/B2/B3 的用户可见错误扩展闭合的 `ErrorCodeSchema` 与
  `protocol.ErrorCode` 并同步双端契约；B6 在 Engine 启动、迁移事务和恢复边界内使用
  `MigrationBackupError` / `MigrationRecoveryError`，未新增 Renderer 可调用方法或 IPC 错误码。
...
### 4.1 错误码扩展（B1/B2/B3；B6 为 Engine 内部错误）
...
B6 自动备份发生在 Sidecar 完成握手之前；备份或恢复失败必须阻止 Engine 就绪。实际实现使用
内部 `MigrationBackupError` / `MigrationRecoveryError`，由现有 Engine 启动失败链向桌面端
报告，不对 Renderer 暴露可滥用的任意 restore 方法，也不新增未使用的 `BACKUP_INCOMPLETE`。
...
- **B6（实际实现）**：迁移前自动执行 Online Backup，数据库外 JSON 元数据含 SHA-256、
  integrity-check、schema 和 outcome，默认保留最近 3 对；迁移失败自动恢复。未暴露 Renderer
  可调用的任意 restore IPC，避免在数据库打开期间形成新的高风险写入口；备份能力本身不再
  修改待保护 schema。
- **B7**：门禁 `finally` 调用清理；文档收口；完整门禁与最终验收。

### 4.4 SQLite 迁移

B1 清理作业表走 `_migrate_schema(current)` 增量路径。B6 必须在该路径之前建立数据库外
备份，因此不增加备份注册表或 schema 版本；保持 `current > SCHEMA_VERSION` 在任何备份
和写入前拒绝。测试覆盖旧→新、拒绝更高版本、迁移失败恢复和恢复失败 fail-closed。
...
- 已用历史提交重建的 `alpha.10` 打包 Engine 实测：它会把 v10 的 `user_version` 改为 v2
  并新增旧表，未满足拒绝合同。结论据实为 PARTIAL，且不修改历史版本伪造通过。
...
- 未引入遥测、远程服务、付费依赖、自动历史删除或应用级加密；B6 已增加本地明文迁移
  备份及恢复边界，设置页会单独显示其占用。
...
### B6（备份）
- 已实现 `sqlite3` Online Backup API；备份后验证源 schema、`integrity_check`、大小与 SHA-256。
- 已实现迁移失败自动校验恢复、恢复失败 fail-closed、WAL 已提交内容备份、原子写入、
  Windows reparse point 拒绝及最近 3 对保留；卸载不自动删除。
- 迁移备份模块 15 项定向测试达到 lines 99.15%、branches 97.92%，并建立不低于
  lines 95% / branches 90% 的独立覆盖率回退预算。
- 设置页已单独展示迁移备份占用并说明其本地明文与保留边界。
- 历史 `alpha.10`（`7d8f3d26`）已从源码重建；打包 Engine 创建的 v2 demo 数据已成功迁移
  到 v10，任务、6 条结果、校对结论、备注、导出记录与文件均保留。
- 历史 `alpha.10` 面对 v10 fixture **没有拒绝**，而是把 `user_version` 改为 v2 并新增
  8 张旧表；因此降级安全合同明确不通过，总体保持 PARTIAL，不修改旧版本伪造通过。
- 仓库无当年可信发布安装器；重建制品不能替代 provenance，记录
...
3. **旧版本遇 future schema**：已用重建的历史 `alpha.10` Engine 实测；其会把 v10 改写为

...
### 4.2 PROTOCOL_VERSION 是否递增（待验证，不预设）

**更正**：不得建立在未经证明的“未知错误码会降级为 `UNKNOWN_ERROR`”假设上。`ErrorCodeSchema` 是**闭合枚举**，未知值在严格解析下会被拒。B0 标记为 **NOT VERIFIED / 待 B1 前置批验证**：

1. **先实测现有失败解析路径**：构造带未知 `error.code` 的响应，观察 TS 端 `ResponseSchema`/`ErrorPayloadSchema.parse` 与 Python 端 `safe_parse`/`make_error` 的实际行为（本轮未做）。
2. **按兼容性事实决定**：仅新增枚举成员且调用方确有兜底 → 不递增；若新增方法名或结果形状改变解析兼容性 → **必须**递增（双端同步）。
3. B0 不下结论；在前置批用契约测试固化该事实后再定。
...
- 临时输出在最终目标同卷同父目录，原子 `os.replace`。
- 目标存在且本轮无 overwrite 参数 → 一律 `OUTPUT_ALREADY_EXISTS`，用户选新路径重试。

### B3（来源预检）
- 网络路径允许但警告+二次确认；默认不跟随 junction/symlink（**须真实临时 junction/symlink 实测确认**）。
- 磁盘检查针对 userData 所在卷；创建时重校验，变化 → `PREFLIGHT_STALE`。
...
4. **PROTOCOL_VERSION 递增决策**：依赖未知错误码解析路径实测，B0 未做（§4.2）。
5. **Setup/Portable 当前签名/制品**：本轮无制品，仅“不存在/未验证”；NotSigned 是策略非实测（§1.8）。
6. **远程 CI**：push 不执行，仅配置审查/本地模拟（§1.10）。
7. **`vertical.spec.ts` 标记与 PS smoke 失败路径**：标记覆盖待 B7 补齐核对（§1.9）。
8. **跨版本升级/回滚、公开许可证批准、正式发布授权**：未验证/未授权（审计 H-REL-01/H-LIC-01）。

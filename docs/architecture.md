...
见 [ipc-protocol.md](ipc-protocol.md)。协议版本 `PROTOCOL_VERSION = 4`，TS（`packages/ipc-schema`，Zod）与 Python（`archivelens_engine.protocol`）双端一致。v4 增加可清空的单条校对决定，以及带 UUID 幂等键、事务化写入和原响应重放的批量校对接口；新界面与旧 v3 引擎不混用。v3 已加入文件夹创建前预检和 `PREFLIGHT_STALE` 错误合同。任务持久化检索词数组与匹配模式：新任务使用单个 `exact_literal` 检索词；旧任务保持 `legacy_fixed_pair` 的“约/約”语义。
...
每条 `occurrences` 在任务内拥有从 1 开始、不可变且唯一的 `global_sequence`。v12 迁移按
来源导入顺序、页码、页内命中顺序和记录 ID 回填；新命中在同一写事务中只向后追加。
筛选、分页、校对和导出都以该序号交叉引用，不因重复写入或后续新增结果而重排旧记录。
...
离线 HTML 以永久序号为默认顺序，每条命中独立展示出处、文字/校对和完整页面；页面图片
数据按页面只序列化一次，各命中卡只叠加自己的标记。报告内置覆盖全部筛选结果的目录，
通过数据索引跨分页定位，不引入外部资源、绝对来源路径或远程脚本。

Renderer 的任务批量操作不扩展 Sidecar 协议：最多选择当前页 20 条，逐项调用现有
`tasks.get` 与单任务暂停、取消、删除方法，并在每次变更前重查状态。这样复用已有状态机、
删除幂等和来源文件保护，不产生并发批量写入的新合同；单项失败会被隔离并可重新核验后重试。
...
命中上下文由 `layout_context.py` 根据同页不可变 OCR 行框构建。它先按空间关系隔离版块，
再在竖排版面选择右邻列、命中列、左邻列，或在横排版面选择上一行、命中行、下一行；
结构化结果同时保存源坐标、归一化坐标、命中字符区间和纯文本索引。自动方向置信不足时
只保留目标行或列并标记 `uncertain`，不会回退到全页文本排序。Renderer、任务内检索和
离线 HTML 复用同一结构。旧任务按当前命中页优先分批重建，人工选择的候选版块或自定义
矩形保存在来源页级覆盖表中，不修改 OCR 原文、永久序号、人工结论或备注。

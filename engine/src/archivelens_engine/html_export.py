...
const state={file:"",status:"",query:"",sort:"source",pageSize:20,page:1,printMode:false,printSnapshot:null,filtered:[],modalIndex:-1};
...
function visibleHits(page){return page.hits.filter((hit)=>{if(state.status&&hit.status!==state.status)return false;if(state.query&&!hitSearchText(page,hit).includes(state.query))return false;return true})}
function pageUrgency(page){return Math.min(...page.visibleHits.map((hit)=>STATUS_RANK[hit.status]??0))}
function filterAndSort(){
  state.filtered=DATA.pages.filter((page)=>!state.file||page.sourceId===state.file).map((page)=>({...page,visibleHits:visibleHits(page)})).filter((page)=>page.visibleHits.length>0);
  const collator=new Intl.Collator("zh-CN",{numeric:true,sensitivity:"base"});
  const source=(a,b)=>a.sourceOrder-b.sourceOrder||a.pageNumber-b.pageNumber;
  const sorters={source,fileAsc:(a,b)=>collator.compare(a.fileName,b.fileName)||a.pageNumber-b.pageNumber,fileDesc:(a,b)=>collator.compare(b.fileName,a.fileName)||a.pageNumber-b.pageNumber,pageAsc:(a,b)=>a.pageNumber-b.pageNumber||source(a,b),pageDesc:(a,b)=>b.pageNumber-a.pageNumber||source(a,b),confidence:(a,b)=>Math.min(...a.visibleHits.map((h)=>h.confidence))-Math.min(...b.visibleHits.map((h)=>h.confidence))||source(a,b),status:(a,b)=>pageUrgency(a)-pageUrgency(b)||source(a,b)};
  state.filtered.sort(sorters[state.sort]||source);
...
function contextNode(hit){const context=el("div","context");if(hit.contextBefore||hit.contextAfter){context.append(document.createTextNode(hit.contextBefore));const mark=el("mark","",hit.matchedText);context.append(mark,document.createTextNode(hit.contextAfter))}else{context.textContent=hit.contextFull||hit.matchedText||"无 OCR 上下文"}return context}
function hitNode(hit,index){const row=el("article","hit");const head=el("div","hit-head");head.append(el("span","hit-index",`命中 ${index+1}`),el("span","match-chip",hit.matchedText||"未识别"));const status=el("span",`status-chip status-${hit.status}`,hit.statusLabel);head.append(status,el("span","confidence",`置信度 ${(hit.confidence*100).toFixed(0)}%`));row.append(head,contextNode(hit));row.append(el("div","note",hit.note?`校对备注：${hit.note}`:"校对备注：无备注"));return row}
function openModal(index){state.modalIndex=index;const page=state.filtered[index];if(!page)return;byId("modal-title").textContent=`${page.fileName} · 第 ${page.pageNumber} 页`;const host=byId("modal-image");host.replaceChildren(imageStage(page,page.visibleHits,true));byId("modal-prev").disabled=index<=0;byId("modal-next").disabled=index>=state.filtered.length-1;byId("image-modal").hidden=false;document.body.classList.add("modal-open")}
...
function cardNode(page,index){const card=el("article","page-card");if(state.printMode&&index>0)card.classList.add("print-break");const head=el("header","card-head");const title=el("div","card-title");title.append(el("strong","",`${page.fileName} · 第 ${page.pageNumber} 页`),el("span","",page.relativePath));const minimum=Math.min(...page.visibleHits.map((hit)=>hit.confidence));head.append(title,el("div","page-summary",`${page.visibleHits.length} 个命中 · 最低置信度 ${(minimum*100).toFixed(0)}%`));card.append(head);const button=el("button","image-button");button.type="button";button.setAttribute("aria-label",`查看 ${page.fileName} 第 ${page.pageNumber} 页大图`);button.append(imageStage(page,page.visibleHits));button.addEventListener("click",()=>openModal(index));card.append(button);const hits=el("section","hits");page.visibleHits.forEach((hit,hitIndex)=>hits.append(hitNode(hit,hitIndex)));card.append(hits);return card}
function renderResults(){filterAndSort();const totalHits=state.filtered.reduce((sum,page)=>sum+page.visibleHits.length,0);byId("result-summary").textContent=`当前显示 ${state.filtered.length} 个命中页面、${totalHits} 条命中记录`;
  ...
  const start=state.printMode?0:(state.page-1)*state.pageSize;const end=state.printMode?state.filtered.length:start+state.pageSize;state.filtered.slice(start,end).forEach((page,localIndex)=>host.append(cardNode(page,start+localIndex)));renderPager()
}
...
function resetFilters(){state.file="";state.status="";state.query="";state.sort="source";state.pageSize=20;state.page=1;byId("file-filter").value="";byId("status-filter").value="";byId("report-search").value="";byId("sort-order").value="source";byId("page-size").value="20";renderResults()}
function preparePrint(){if(state.printMode)return;state.printSnapshot={file:state.file,status:state.status,query:state.query,sort:state.sort,pageSize:state.pageSize,page:state.page};state.file="";state.status="";state.query="";state.sort="source";state.page=1;state.printMode=true;renderResults()}
...
byId("print-report").addEventListener("click",()=>{if(window.confirm(`将打印完整报告，共包含 ${DATA.pageCount} 个命中页面。当前筛选条件不会影响打印内容。`))window.print()});
...
<div id="results-start" class="toolbar"><div class="control"><label for="file-filter">文件筛选</label><select id="file-filter"><option value="">全部文件</option></select></div><div class="control"><label for="status-filter">校对状态</label><select id="status-filter"><option value="">全部状态</option><option value="unreviewed">未校对</option><option value="needs_review">待复核</option><option value="rejected">已拒绝</option><option value="confirmed">已确认</option></select></div><div class="control search"><label for="report-search">关键词搜索</label><input id="report-search" type="search" placeholder="搜索文件、路径、命中、上下文或备注"></div><div class="control"><label for="sort-order">结果排序</label><select id="sort-order"><option value="source">文件顺序 + 页码升序</option><option value="fileAsc">文件名升序</option><option value="fileDesc">文件名降序</option><option value="pageAsc">页码升序</option><option value="pageDesc">页码降序</option><option value="confidence">最低置信度优先</option><option value="status">校对状态优先</option></select></div><div class="control"><label for="page-size">每页页面卡片</label><select id="page-size"><option value="10">10</option><option value="20" selected>20</option><option value="50">50</option><option value="100">100</option></select></div><button id="reset-filters" class="secondary-button" type="button">重置</button></div>
<p id="result-summary" class="result-summary" aria-live="polite"></p><div id="result-list" class="result-list"></div><nav id="pager" class="pager" aria-label="报告分页"><button id="first-page" class="page-button" type="button">首页</button><button id="prev-page" class="page-button" type="button">上一页</button><span id="page-indicator" class="page-indicator"></span><button id="next-page" class="page-button" type="button">下一页</button><button id="last-page" class="page-button" type="button">末页</button></nav></section></main>
<button id="back-top" class="back-top" type="button" hidden>返回顶部</button><section id="image-modal" class="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" hidden><header class="modal-head"><strong id="modal-title"></strong><button id="modal-close" class="modal-button modal-close" type="button" aria-label="关闭大图">关闭</button></header><div id="modal-image" class="modal-stage"></div><footer class="modal-actions"><button id="modal-prev" class="modal-button" type="button">上一张</button><button id="modal-next" class="modal-button" type="button">下一张</button></footer></section><script>"""

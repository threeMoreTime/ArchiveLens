...
def _layout_context_data(item: dict[str, Any]) -> dict[str, Any] | None:
    raw = item.get("layout_context") or item.get("layout_context_json")
...
def _hit_data(item: dict[str, Any], index: int) -> dict[str, Any]:
    ...
    return {
        ...
        "layoutContext": _layout_context_data(item),
...
.layout-context{display:grid;gap:8px;margin-top:10px}.layout-context-head{display:flex;align-items:baseline;justify-content:space-between;gap:8px}.layout-context-head strong{font-size:14px}.layout-context-head span{color:var(--muted);font-size:12px}.layout-warning{padding:7px 9px;border:1px solid #dfbf7a;border-radius:7px;background:#fff6e4;color:#6d4300;font-size:12px}.layout-viewport{max-height:620px;overflow:auto;border:1px solid #d9ccb7;border-radius:8px;background:#f3ead8}.layout-canvas{position:relative;min-width:100%;min-height:100%;background-image:linear-gradient(rgba(125,88,45,.035) 1px,transparent 1px);background-size:100% 32px;color:#251c14;font-family:"FangSong","STFangsong","Songti SC","SimSun",serif}.layout-line{position:absolute;display:block;overflow:visible;font-size:21px;line-height:1.65;white-space:pre}.layout-viewport.vertical .layout-line{font-size:25px;letter-spacing:.09em;line-height:1.5;text-orientation:upright;writing-mode:vertical-rl}.layout-line mark{padding:0 1px;border-radius:2px;background:var(--highlight);color:inherit}@media print{.layout-viewport{max-height:none;overflow:visible}.layout-context{break-inside:avoid}}
/* 命中记录模式：导航和正文共享永久序号，正文严格按出处、文字、原图排列。 */
.review-layout{display:grid;grid-template-columns:320px minmax(0,1fr);gap:20px;align-items:start;transition:grid-template-columns 180ms ease,gap 180ms ease}.review-layout.nav-collapsed{grid-template-columns:0 minmax(0,1fr);gap:0}.record-nav{position:sticky;top:96px;width:320px;max-height:calc(100vh - 116px);overflow:hidden;border:1px solid var(--line);border-radius:12px;background:#fff;box-shadow:0 8px 28px rgba(40,30,15,.06);opacity:1;transform:translateX(0);transition:opacity 140ms ease,transform 180ms ease,visibility 0s}.nav-collapsed .record-nav{visibility:hidden;pointer-events:none;opacity:0;transform:translateX(-12px);transition:opacity 120ms ease,transform 180ms ease,visibility 0s linear 180ms}.record-nav-head{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:12px;border-bottom:1px solid var(--line);background:#faf7f2}.record-nav-title{display:flex;align-items:center;gap:8px;margin:0;font-size:16px}.record-nav-count{display:inline-grid;min-width:24px;height:24px;place-items:center;border-radius:999px;background:#f4ead9;color:#7a4b13;font-size:12px;font-weight:800;font-variant-numeric:tabular-nums}.record-nav-count[hidden]{display:none}.record-nav-toggle,.record-nav-reveal{display:inline-grid;width:38px;height:38px;place-items:center;padding:0;border:1px solid #cfc6b8;border-radius:9px;background:#fff;color:#7a4b13;cursor:pointer;font:700 24px/1 "Segoe UI Symbol","Microsoft YaHei",sans-serif}.record-nav-toggle:hover,.record-nav-reveal:hover,.record-nav-mobile-toggle:hover{border-color:#9c6b2f;background:#fff7ed}.record-nav-toggle:focus-visible,.record-nav-reveal:focus-visible,.record-nav-mobile-toggle:focus-visible,.record-nav-item:focus-visible{outline:3px solid rgba(180,83,9,.35);outline-offset:2px}.record-nav-reveal{position:fixed;top:110px;left:max(4px,calc((100vw - min(1440px,100vw))/2 + 4px));z-index:9;display:none;width:42px;height:42px;box-shadow:0 8px 22px rgba(40,30,15,.18)}.nav-collapsed .record-nav-reveal{display:inline-grid}.record-nav-reveal .record-nav-count{position:absolute;right:-7px;top:-7px;min-width:21px;height:21px;padding:0 5px;border:2px solid #fff;background:#b45309;color:#fff;font-size:11px}.record-nav-mobile-toggle{display:none;width:100%;min-height:46px;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;border:0;background:#faf7f2;color:var(--ink);cursor:pointer;font:700 15px/1.3 "Microsoft YaHei","Segoe UI",sans-serif;text-align:left}.record-nav-mobile-label{display:flex;align-items:center;gap:8px}.record-nav-mobile-icon{color:#7a4b13;font:700 22px/1 "Segoe UI Symbol","Microsoft YaHei",sans-serif}.record-nav-list{max-height:calc(100vh - 178px);overflow:auto;padding:8px}.record-nav-item{display:grid;width:100%;gap:3px;padding:10px;border:0;border-left:3px solid transparent;border-radius:8px;background:transparent;color:var(--ink);font:inherit;text-align:left;cursor:pointer}.record-nav-item:hover{background:#fff7ed}.record-nav-item.is-current{border-left-color:#b45309;background:#fff7ed;box-shadow:inset 0 0 0 1px rgba(180,83,9,.12)}.record-nav-item+.record-nav-item{border-top:1px solid var(--line)}.record-nav-item strong,.record-nav-path{overflow-wrap:anywhere}.record-nav-path{color:var(--muted);font-size:12px}.record-nav-match{color:#8b1d1d;font-size:13px;font-weight:700}.record-nav-context{display:-webkit-box;overflow:hidden;color:var(--muted);font-size:12px;-webkit-box-orient:vertical;-webkit-line-clamp:2}.result-pane{min-width:0}.occurrence-card.targeted{outline:3px solid #f59e0b;outline-offset:3px;animation:target-pulse 1.8s ease-out}.source-sequence{color:#8b4513}.record-text{padding:18px;border-bottom:1px solid var(--line)}.record-section-title{margin:0 0 10px;font-size:15px}.record-meta{display:flex;align-items:center;flex-wrap:wrap;gap:9px;margin-bottom:10px}@keyframes target-pulse{0%{box-shadow:0 0 0 8px rgba(245,158,11,.25)}100%{box-shadow:0 8px 28px rgba(40,30,15,.07)}}
@media(max-width:900px){.review-layout,.review-layout.nav-collapsed{grid-template-columns:minmax(0,1fr);gap:20px}.record-nav,.nav-collapsed .record-nav{position:static;width:auto;max-height:none;visibility:visible;pointer-events:auto;opacity:1;transform:none;transition:none}.record-nav-head{display:none}.record-nav-mobile-toggle{display:flex}.record-nav-list{max-height:38vh}.nav-collapsed .record-nav-list{display:none}.record-nav-reveal,.nav-collapsed .record-nav-reveal{display:none}}
@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}.review-layout,.record-nav,.nav-collapsed .record-nav{transition:none}.occurrence-card.targeted{animation:none}}
@media print{.record-nav,.record-nav-toggle,.record-nav-reveal,.record-nav-mobile-toggle{display:none!important}.review-layout,.review-layout.nav-collapsed{display:block}.occurrence-card{break-inside:avoid}.occurrence-card.print-break{break-before:page}.record-text{padding:3mm 4mm}.image-button{order:3}}
""".strip()


REPORT_SCRIPT = r"""
const STATUS_RANK={unreviewed:0,needs_review:1,rejected:2,confirmed:3};
const state={file:"",status:"",query:"",sort:"sequence",pageSize:20,page:1,printMode:false,printSnapshot:null,filtered:[],modalIndex:-1,navCollapsed:false,activeRecordId:""};
...
function hitSearchText(page,hit){return normalized([page.fileName,page.relativePath,hit.matchedText,hit.contextBefore,hit.contextAfter,hit.contextFull,hit.layoutContext?.plain_text,hit.note].join(" "))}
function recordId(record){return `record-${record.sequence}-${record.ordinal}`}
function recordContext(hit){return hit.layoutContext?.plain_text||hit.contextFull||`${hit.contextBefore||""}${hit.matchedText||""}${hit.contextAfter||""}`||hit.matchedText||"无 OCR 上下文"}
...
function layoutContextNode(hit){const context=hit.layoutContext;if(!context||!Array.isArray(context.items)||!context.items.length)return contextNode(hit);const vertical=context.orientation==="vertical";const section=el("section","layout-context");const head=el("div","layout-context-head");head.append(el("strong","","版面 OCR 上下文"),el("span","",vertical?"命中列及相邻两列":"命中行及相邻两行"));section.append(head);if(context.status==="uncertain")section.append(el("div","layout-warning",`版面结构待确认，当前只显示命中${vertical?"列":"行"}。`));const viewport=el("div",`layout-viewport ${vertical?"vertical":"horizontal"}`);const canvas=el("div","layout-canvas");const advances=context.items.map((item)=>{const box=item.bbox||{};const span=vertical?Number(box.y1)-Number(box.y0):Number(box.x1)-Number(box.x0);return item.text?span/String(item.text).length:0}).filter((value)=>Number.isFinite(value)&&value>0).sort((a,b)=>a-b);const sourceAdvance=advances[Math.floor(advances.length/2)]||12;const scale=Math.min(4.5,Math.max(.75,(vertical?31:25)/sourceAdvance));const padding=28;const box=context.bbox||{x0:0,y0:0,x1:1,y1:1};const width=Math.max(vertical?260:520,(Number(box.x1)-Number(box.x0))*scale+padding*2);const height=Math.max(vertical?520:180,(Number(box.y1)-Number(box.y0))*scale+padding*2);canvas.style.width=`${width}px`;canvas.style.height=`${height}px`;context.items.forEach((item)=>{const itemBox=item.bbox||box;const line=el("span",`layout-line ${item.role||"context"}`);line.style.left=`${padding+(Number(itemBox.x0)-Number(box.x0))*scale}px`;line.style.top=`${padding+(Number(itemBox.y0)-Number(box.y0))*scale}px`;line.style.minWidth=`${Math.max(24,(Number(itemBox.x1)-Number(itemBox.x0))*scale)}px`;line.style.minHeight=`${Math.max(24,(Number(itemBox.y1)-Number(itemBox.y0))*scale)}px`;const text=String(item.text||"");const start=item.role==="target"?Number(item.match_start):-1;const end=item.role==="target"?Number(item.match_end):-1;if(Number.isInteger(start)&&Number.isInteger(end)&&start>=0&&end>start&&end<=text.length)line.append(document.createTextNode(text.slice(0,start)),el("mark","",text.slice(start,end)),document.createTextNode(text.slice(end)));else line.textContent=text;canvas.append(line)});viewport.append(canvas);section.append(viewport);return section}
function hitNode(hit){const row=el("section","record-text");row.append(el("h3","record-section-title","OCR 文字与校对信息"));const meta=el("div","record-meta");meta.append(el("span","match-chip",hit.matchedText||"未识别"),el("span",`status-chip status-${hit.status}`,hit.statusLabel),el("span","confidence",`OCR 置信度 ${(hit.confidence*100).toFixed(0)}%`));row.append(meta,layoutContextNode(hit),el("div","note",hit.note?`校对备注：${hit.note}`:"校对备注：无备注"));return row}
...
function recordNavItem(record){const id=recordId(record);const button=el("button","record-nav-item");button.type="button";button.dataset.recordId=id;button.title=record.page.relativePath;if(id===state.activeRecordId){button.classList.add("is-current");button.setAttribute("aria-current","location")}button.append(el("strong","",`${sequenceLabel(record.sequence)} · ${record.page.fileName} · 第 ${record.page.pageNumber} 页`),el("span","record-nav-match",`命中：${record.hit.matchedText||"未识别"}`),el("span","record-nav-context",recordContext(record.hit)));button.addEventListener("click",()=>navigateToRecord(record));return button}
function setCountBadge(id,count){const badge=byId(id);badge.hidden=count===0;badge.textContent=String(count)}
function renderNavigation(){const host=byId("record-nav-list");host.replaceChildren();state.filtered.forEach((record)=>host.append(recordNavItem(record)));if(!state.filtered.length)host.append(el("p","empty","当前筛选没有命中记录。"));const count=state.filtered.length;setCountBadge("record-nav-count",count);setCountBadge("record-nav-reveal-count",count);setCountBadge("record-nav-mobile-count",count);const countText=count?`，共 ${count} 条命中记录`:"，当前无匹配记录";byId("record-nav-toggle").setAttribute("aria-label",`收起命中导航${countText}`);byId("record-nav-reveal").setAttribute("aria-label",`展开命中导航${countText}`);byId("record-nav-mobile-toggle").setAttribute("aria-label",`${state.navCollapsed?"展开":"收起"}命中导航${countText}`)}
function ensureActiveRecord(records){if(!records.some((record)=>recordId(record)===state.activeRecordId))state.activeRecordId=records.length?recordId(records[0]):""}
function updateActiveNavigation(id,follow=false){if(!id)return;state.activeRecordId=id;const host=byId("record-nav-list");let active=null;host.querySelectorAll(".record-nav-item").forEach((item)=>{const current=item.dataset.recordId===id;item.classList.toggle("is-current",current);if(current){item.setAttribute("aria-current","location");active=item}else item.removeAttribute("aria-current")});if(!follow||state.navCollapsed||!active)return;const top=active.offsetTop;const bottom=top+active.offsetHeight;if(top<host.scrollTop)host.scrollTo({top:Math.max(0,top-8),behavior:prefersReducedMotion()?"auto":"smooth"});else if(bottom>host.scrollTop+host.clientHeight)host.scrollTo({top:bottom-host.clientHeight+8,behavior:prefersReducedMotion()?"auto":"smooth"})}
function syncActiveFromScroll(){if(state.printMode)return;const cards=[...document.querySelectorAll(".occurrence-card")];if(!cards.length)return;const anchor=120;let active=cards.find((card)=>card.getBoundingClientRect().bottom>anchor);if(!active)active=cards[cards.length-1];updateActiveNavigation(active.id,true)}
let activeScrollFrame=0;function scheduleActiveSync(){if(activeScrollFrame)return;activeScrollFrame=requestAnimationFrame(()=>{activeScrollFrame=0;syncActiveFromScroll()})}
function navigateToRecord(record){const index=state.filtered.indexOf(record);if(index<0)return;state.page=Math.floor(index/state.pageSize)+1;state.activeRecordId=recordId(record);renderResults();requestAnimationFrame(()=>{const target=byId(recordId(record));if(!target)return;updateActiveNavigation(target.id,true);target.scrollIntoView({behavior:prefersReducedMotion()?"auto":"smooth",block:"start"});target.classList.add("targeted");setTimeout(()=>target.classList.remove("targeted"),1900)})}
function setNavCollapsed(collapsed){state.navCollapsed=Boolean(collapsed);byId("review-layout").classList.toggle("nav-collapsed",state.navCollapsed);const expanded=String(!state.navCollapsed);byId("record-nav-toggle").setAttribute("aria-expanded",expanded);byId("record-nav-reveal").setAttribute("aria-expanded",expanded);byId("record-nav-mobile-toggle").setAttribute("aria-expanded",expanded);byId("record-nav-mobile-icon").textContent=state.navCollapsed?"⌄":"⌃";renderNavigation();if(!state.navCollapsed)requestAnimationFrame(()=>updateActiveNavigation(state.activeRecordId,true))}
function renderResults(){filterAndSort();const start=state.printMode?0:(state.page-1)*state.pageSize;const end=state.printMode?state.filtered.length:start+state.pageSize;const visibleRecords=state.filtered.slice(start,end);ensureActiveRecord(visibleRecords);renderNavigation();const pageCount=new Set(state.filtered.map((record)=>record.page.id)).size;byId("result-summary").textContent=`当前显示 ${state.filtered.length} 条命中记录，分布在 ${pageCount} 个页面`;
  ...
  visibleRecords.forEach((record,localIndex)=>host.append(cardNode(record,start+localIndex)));renderPager();requestAnimationFrame(scheduleActiveSync)
}
...
function goPage(value){const pages=Math.max(1,Math.ceil(state.filtered.length/state.pageSize));state.page=Math.min(pages,Math.max(1,value));state.activeRecordId="";renderResults();byId("results-start").scrollIntoView({behavior:prefersReducedMotion()?"auto":"smooth",block:"start"})}
...
byId("record-nav-reveal").addEventListener("click",()=>setNavCollapsed(false));
byId("record-nav-mobile-toggle").addEventListener("click",()=>setNavCollapsed(!state.navCollapsed));
...
window.addEventListener("beforeprint",preparePrint);window.addEventListener("afterprint",restoreAfterPrint);window.addEventListener("scroll",()=>{byId("back-top").hidden=window.scrollY<500;scheduleActiveSync()},{passive:true});window.addEventListener("resize",scheduleActiveSync,{passive:true});byId("back-top").addEventListener("click",()=>window.scrollTo({top:0,behavior:prefersReducedMotion()?"auto":"smooth"}));
""".strip()
...
<p id="result-summary" class="result-summary" aria-live="polite"></p><div id="review-layout" class="review-layout"><aside id="record-nav" class="record-nav"><div class="record-nav-head"><h2 class="record-nav-title">命中导航 <span id="record-nav-count" class="record-nav-count" hidden>0</span></h2><button id="record-nav-toggle" class="record-nav-toggle" type="button" aria-expanded="true" aria-controls="record-nav-list" aria-label="收起命中导航" title="收起命中导航"><span aria-hidden="true">‹</span></button></div><button id="record-nav-mobile-toggle" class="record-nav-mobile-toggle" type="button" aria-expanded="true" aria-controls="record-nav-list" aria-label="收起命中导航"><span class="record-nav-mobile-label">命中导航 <span id="record-nav-mobile-count" class="record-nav-count" hidden>0</span></span><span id="record-nav-mobile-icon" class="record-nav-mobile-icon" aria-hidden="true">⌃</span></button><nav id="record-nav-list" class="record-nav-list" aria-label="全部筛选结果导航"></nav></aside><button id="record-nav-reveal" class="record-nav-reveal" type="button" aria-expanded="true" aria-controls="record-nav-list" aria-label="展开命中导航" title="展开命中导航"><span aria-hidden="true">›</span><span id="record-nav-reveal-count" class="record-nav-count" hidden>0</span></button><div class="result-pane"><div id="result-list" class="result-list"></div><nav id="pager" class="pager" aria-label="报告分页"><button id="first-page" class="page-button" type="button">首页</button><button id="prev-page" class="page-button" type="button">上一页</button><span id="page-indicator" class="page-indicator"></span><button id="next-page" class="page-button" type="button">下一页</button><button id="last-page" class="page-button" type="button">末页</button></nav></div></div></section></main>
